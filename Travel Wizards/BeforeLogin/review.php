...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $userid = $_SESSION['user_id'] ?? null; // Assuming the session variable is 'user_id'

    if (!$userid) {
        // If no user_id is set, show an error and exit
        $showPopup = true;
        $popupMessage = "You must be logged in to leave a review.";
    }
    ...
    if (!empty($userid) && !empty($rating) && !empty($service) && !empty($reviewText)) {
        try {
            // Insert review into the database (make sure the 'reviews' table has 'userid' column)
